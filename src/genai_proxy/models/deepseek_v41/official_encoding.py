"""Vendored DeepSeek-V4.1 official text/vision encoding (prompt format reference).

Copied verbatim from deepseek-ai/DeepSeek-V4.1-Flash, revision dba1be0a40aa,
encoding/encoding.py (sha256 502bdaec8a3fd88ebc24c4721a7038fbe42f2063c664638127056107920035c1).

Hardcoded on purpose: GenAI's encoding rules are stable and rarely change, so
the prompt codec ships with the proxy instead of being downloaded at runtime.

WARNING: keep this file byte-identical to the upstream release below the
header so prompt rendering and token accounting stay reproducible.
"""
